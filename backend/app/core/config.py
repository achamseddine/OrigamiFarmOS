...
`local`/`demo` mode and PostgreSQL in `pilot`/`staging`/`production`
...
class Settings:
    ...
    default_farm_id: str = "farm-origami"
    demo_mode: bool = os.environ.get("FARMOS_DEMO_MODE", "true").lower() == "true"
